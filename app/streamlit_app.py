...
selected_genes = st.sidebar.multiselect(
    "Gene symbols", gene_options, default=scores.head(3).index.tolist()
)
...
def adc_heuristic(row: pd.Series) -> tuple[str, str]:
    MIN_ADC_LENGTH = 200
    MAX_ADC_LENGTH = 800
    MIN_SURFACE_LENGTH = 120
    MAX_SURFACE_LENGTH = 1500
    ...
    if surface and signal and ig and MIN_ADC_LENGTH <= length <= MAX_ADC_LENGTH:
        return "green", "Meets key ADC-friendly hallmarks"
    if surface and signal and MIN_SURFACE_LENGTH <= length <= MAX_SURFACE_LENGTH:
