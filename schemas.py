...
class APIPredictionRequest(BaseModel):
    ...
    Features are validated against the model's expected feature schema.
    Use the validation_mode field to control strictness:
    - strict: All features must match exactly
    - lenient: Missing features are errors, extra features are warnings (default)
    - permissive: Missing features filled with defaults, extra ignored
    """
    ...
    features: Dict[str, float] = Field(
        ...,
        description="Dictionary of feature name to numeric value",
        examples=[{"ppi_degree": 0.5, "domain_count": 3.0}],
    )
    model_version: Optional[str] = Field(
        default=None,
        description="Optional model version to use for prediction",
    )
    validation_mode: Optional[Literal["strict", "lenient", "permissive"]] = Field(
        default="lenient",
        description="Feature validation strictness: strict, lenient, or permissive",
    )
...
class APIPredictionResponse(BaseModel):
    ...
    validation_warnings: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="Any warnings from feature validation",
    )
...
class APIExplanationResponse(BaseModel):
    ...
    model_version: str = Field(
        ...,
        description="Version of the model used for explanation",
    )
...
class APIValidationErrorResponse(BaseModel):
    ...
    validation_errors: List[Dict[str, Any]] = Field(
        ...,
        description="List of validation errors with details",
    )


class APIFeatureInfoResponse(BaseModel):
    """Schema for feature information endpoint response."""

    model_config = ConfigDict(protected_namespaces=())

    total_features: int = Field(
        ...,
        description="Total number of expected features",
    )
    feature_names: List[str] = Field(
        ...,
        description="Ordered list of expected feature names",
    )
    feature_specs: Optional[Dict[str, Dict[str, Any]]] = Field(
        default=None,
        description="Detailed specifications for each feature",
    )
