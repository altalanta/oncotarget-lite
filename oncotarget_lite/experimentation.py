...
class ExperimentConfig:
    ...
    metrics: List[str] # Removed default None
    ...
    # Removed __post_init__ as metrics will always be provided by Hydra
    ...
    def optimize_with_experiment(
        ...
        try:
            ...
            if experiment.best_trial:
                ...
                typer.echo(f"   Model: {experiment.best_trial.parameters.get('model_type', 'unknown')}")
...
def run_experiment_cmd(
    ...
    # config_file: Optional[Path] = typer.Option(None, help="JSON file with experiment configuration"), # Removed
    ...
    # if config_file and config_file.exists(): # Removed old config loading
    #     try:
    ...
    #     search_spaces = { # Removed default search spaces
    #         "logreg": {
    ...
    # Create ExperimentConfig from Hydra config
    config = ExperimentConfig(
        experiment_name=cfg.experiment.experiment_name,
        model_types=list(cfg.experiment.model_types),
        search_spaces=OmegaConf.to_container(cfg.experiment.search_spaces, resolve=True),
        metrics=list(cfg.experiment.metrics),
        optimization_direction=cfg.experiment.optimization_direction,
...
def experiment_report_cmd(
    ...
    if experiment.best_trial:
        ...
        best_score = experiment.best_trial.metrics.get(primary_metric, 0)
        typer.echo("🏆 Best Result:")
        typer.echo(f"   {primary_metric.upper()}: {best_score:.4f}")
